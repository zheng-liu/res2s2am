...
def make_kwargs(filename_test_kwargs):
    # test_kwargs, cnn_1s
    test_kwargs_cnn_1s = {}
    test_kwargs_cnn_1s['random_seed'] = 1337
    test_kwargs_cnn_1s['cudnn'] = True
    test_kwargs_cnn_1s['batch_size'] = 256
    test_kwargs_cnn_1s['encode_mode'] = 'N_2_zero'
    test_kwargs_cnn_1s['shuffle'] = True
    test_kwargs_cnn_1s['num_workers'] = 8
    pickle.dump(test_kwargs_cnn_1s, open(filename_test_kwargs, 'wb'))
    print('[INFO] {} dumped!'.format(filename_test_kwargs))
    return test_kwargs_cnn_1s
...
if __name__ == '__main__':
    ...
    model.train(False)
    testset = GenomeDataset(filename=filename_testset, seq_name=['seq_ref_1'], encode_mode=encode_mode)
    ...
    for data in tqdm(testloader):
        inputs, labels, rsids, query_snp_rsids = data
