...
class resnet_2s2a_metadata(nn.Module):
    # def __init__(self, block, layers, seqlength=1000, num_classes=2):
    def __init__(self, kwargs):
        super(resnet_2s2a_metadata, self).__init__()
        block = kwargs['block']
        ...
        self.inlength_meta = len(kwargs['metadata_cols'])
        self.outlength_meta = kwargs['metadata_fc']
        self.fc_meta = nn.Linear(self.inlength_meta, self.outlength_meta)
        self.fc = nn.Linear(128 * block.expansion * self.outlength_ref * 2 + self.outlength_meta, num_classes)
    ...
    def forward(self, x1, x2, x3, x4, x_meta):
        ...
        x_meta = x_meta.view(-1, self.num_flat_features(x_meta))
        x_meta = self.fc_meta(x_meta)
        x = torch.cat([x, x_meta], 1)
